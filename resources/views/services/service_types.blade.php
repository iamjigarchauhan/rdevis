				...
				for($i=0; $i<$cnt; $i++)
				{
				 ?>
					  	...
					  	<div class="images-box col-sm-4 col-md-4">
							<a class="gallery-images" rel="fancybox" href="{{url('/services/'.$data[$i]['id'])}}">
							  <img src="{{URL::to('/public').Storage::url($data[$i]['image'])}}" width="370" height="270" alt="">
							  <span class="bg-images"><!--<i class="fa fa-search"></i>--></span>
							</a>

					   		<button type="button" onclick="window.location.href= `<?= url('/services/'.$data[$i]['id']) ?>`" class="btn btn-primary btn-lg btn-block">{{$data[$i]['name']}}</button>
